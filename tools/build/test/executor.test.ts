...
function copyRule(env: Env, input: string, out: string,
...
test('a failed rule records nothing and fails the build', async () => {
    let env = setup();
    rule(src(env, 'a.txt', 'x'), ['false'], 'mid.txt');
    let result = await runBuild(env);
    assert.ok(!result.ok);
    assert.deepEqual(statuses(result), ['failed']);
    let store = new Store(env.dbPath);
...
test('dry run reports without writing state', async () => {
    let env = setup();
    copyRule(env, src(env, 'a.txt', 'dry'), 'one.txt');
    copyRule(env, src(env, 'b.txt', 'dry too'), 'two.txt');
    let result = await runBuild(env, {dryRun: true});
    assert.ok(!result.ok);
    assert.deepEqual(result.outcomes.get(getDecls()[0]!), {status: 'would-run', reason: 'new'});
    assert.deepEqual(result.outcomes.get(getDecls()[1]!), {status: 'would-run', reason: 'new'});
...
test('a subset build touches only the decls it is given', async () => {
    let env = setup();
    copyRule(env, src(env, 'a.txt', 'skipped'), 'a.txt');
    let picked = copyRule(env, src(env, 'b.txt', 'solo'), 'b.txt');
    let result = await runBuild(env, {}, [picked.decl]);
    assert.ok(result.ok);
    assert.equal(execCount(env), 1);
    assert.equal(fs.readFileSync(casPath(env.casDir, picked.hash, 'txt'), 'utf8'), 'solo');
});
