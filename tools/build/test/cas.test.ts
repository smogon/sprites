...
import {casExists, casInsert, casPath, casSweep} from '../cas.ts';
...
test('casInsert stores by content digest, read-only', () => {
    ...
    const digest = casInsert(cas, stage(root, 'hello'), 'png');
    ...
    assert.ok(!casExists(cas, digest, 'gif'));
...
test('casInsert dedupes an existing object and discards the temp', () => {
    const root = makeTmpRoot();
    const cas = pathlib.join(root, 'cas');
    const d1 = casInsert(cas, stage(root, 'same'), 'png');
    const tmp2 = stage(root, 'same');
    const d2 = casInsert(cas, tmp2, 'png');
    assert.equal(d1, d2);
    assert.ok(!fs.existsSync(tmp2));
    // Same bytes under a different extension is a distinct object.
    const d3 = casInsert(cas, stage(root, 'same'), 'gif');
    assert.equal(d1, d3);
...
test('casSweep removes non-live objects and prunes empty fanout dirs', () => {
    const root = makeTmpRoot();
    const cas = pathlib.join(root, 'cas');
    const keep = casInsert(cas, stage(root, 'keep'), 'png');
    const drop = casInsert(cas, stage(root, 'drop'), 'png');
